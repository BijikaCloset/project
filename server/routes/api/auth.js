...
router.post("/register", (req, res) => {
  ...
  if (!address) {
    return res
      .status(400)
      .json({ error: "You must enter a Address for shipping." });
  }

  if (!city) {
    return res
      .status(400)
      .json({ error: "You must enter a city for shipping." });
  }

  if (!zipcode) {
    return res.status(400).json({ error: "You must enter a zipcode." });
  }
